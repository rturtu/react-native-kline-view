...
class HTDrawContext {
    ...
    func touchesGesture(
        _ location: CGPoint, _ translation: CGPoint, _ state: UIGestureRecognizerState
    ) {
        ...
        case .began:
            if configManager.shouldReloadDrawItemIndex > HTDrawState.showContext.rawValue {
                ...
                if selectedDrawItem.pointList.count >= selectedDrawItem.drawType.count {
                    if HTDrawItem.canResponseLocation(drawItemList, location, klineView)
                        != selectedDrawItem
                    {
        ...
        if HTDrawItem.canResponseTouch(drawItemList, location, translation, state, klineView) {
            if state == .began, let moveItem = HTDrawItem.findTouchMoveItem(drawItemList),
        ...
        case .began:
            if drawItem == nil
                || (drawItem?.pointList.count ?? 0) >= (drawItem?.drawType.count ?? 0)
            {
                ...
                if case .ended = state, let drawItem = drawItem {
                    ...
                    if index == drawItem.drawType.count - 1 {
                        ...
                        if configManager.drawShouldContinue {
                            configManager.shouldReloadDrawItemIndex =
                                HTDrawState.showContext.rawValue
                        } else {
    ...
    func drawLine(
        _ context: CGContext, _ drawItem: HTDrawItem, _ startPoint: CGPoint, _ endPoint: CGPoint
    ) {
    ...
    func drawMapper(_ context: CGContext, _ drawItem: HTDrawItem, _ index: Int, _ itemIndex: Int) {
        ...
        if index == 2, case .parallelLine = drawItem.drawType,
            let (startPoint, endPoint) = lineList.first
        {
        ...
        for (startPoint, endPoint) in lineList {
            drawLine(
                context, drawItem, klineView.viewPointFromValuePoint(startPoint),
                klineView.viewPointFromValuePoint(endPoint))
        }
        ...
        context.addArc(
            center: klineView.viewPointFromValuePoint(point), radius: 10, startAngle: 0,
            endAngle: CGFloat(Double.pi * 2.0), clockwise: true)
        context.setFillColor(drawItem.drawColor.withAlphaComponent(0.5).cgColor)
        context.drawPath(using: .fill)
        context.addArc(
            center: klineView.viewPointFromValuePoint(point), radius: 4, startAngle: 0,
            endAngle: CGFloat(Double.pi * 2.0), clockwise: true)
